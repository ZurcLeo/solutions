-- =====================================================
-- MIGRAÇÃO: Gifts — Campo message + sticker_image_url no feed
-- Épicos: GIFT-UX-001 (message) + GIFT-UX-002 (imagem desnormalizada)
-- Data: 2026-05-25
-- =====================================================

-- =====================================================
-- 1. GIFT-UX-001: Coluna message na tabela gifts
--    Permite ao presenteador incluir uma mensagem curta
--    junto com o sticker enviado.
-- =====================================================
ALTER TABLE gifts
  ADD COLUMN IF NOT EXISTS message TEXT
  CHECK (message IS NULL OR char_length(message) <= 100);

COMMENT ON COLUMN gifts.message IS
  'Mensagem opcional do remetente ao enviar o presente (máx. 100 chars).';

-- =====================================================
-- 2. GIFT-UX-001: Atualizar RPC send_sticker_gift para
--    aceitar e persistir o campo message.
--    Parâmetro p_message com DEFAULT NULL mantém
--    retrocompatibilidade com callers que não passam o campo.
-- =====================================================
...
BEGIN
    ...
    -- -----------------------------------------------
    -- 6. INSERIR gift record — inclui message (trunca se > 100)
    -- -----------------------------------------------
...
COMMENT ON FUNCTION send_sticker_gift(TEXT, TEXT, UUID, TEXT, TEXT, TEXT) IS
    ...
    '→ debit/credit via spend_elo_coins (FOR UPDATE) → insert gifts (com message) + user_stickers. '
