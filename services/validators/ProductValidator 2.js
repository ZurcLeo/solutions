...
const TYPE_RULES = {
  ...
  property: {
    ...
    validate(data) {
      if (data.property_type_slug != null) {
        const valid = ['apartamento','casa','terreno','sala_comercial','kitnet','cobertura','sobrado','chacara','galpao','loja'];
        if (!valid.includes(data.property_type_slug)) {
          throw new Error(`property_type_slug inválido: ${data.property_type_slug}`);
        }
      }
    },
  },
